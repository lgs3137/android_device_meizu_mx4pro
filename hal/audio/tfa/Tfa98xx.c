...
Tfa98xx_DspGetSwFeatureBits(Tfa98xx_handle_t handle, int features[2])
{
    ...
    error =
        Tfa98xx_DspGetParam(handle, MODULE_FRAMEWORK, FW_PARAM_GET_FEATURE_BITS, sizeof(bytes), bytes);
    if(error != Tfa98xx_Error_Ok)
    {
    ...
    for(i = 0; i < MAX_HANDLES; ++i)
    {
        if(!handlesLocal[i].in_use)
        {
            switch(slave_address)
            {
                ...
                case 0x6C:
                case 0x6E:
#define I2C_PATH "/dev/i2c-68"
                    NXP_I2C_Open(I2C_PATH, slave_address);
...
#define dateLen (13)
    ...
    error =
        Tfa98xx_DspGetParam(handle, 1, 0xFF, dateLen * 3, (unsigned char *)buffer);
    if(error != Tfa98xx_Error_Ok)
        return error;
...
enum Tfa98xx_Error Tfa98xx_Init(Tfa98xx_handle_t handle)
{
    ...
    error = Tfa98xx_WriteRegister16(handle, TFA98XX_SYS_CTRL, TFA98XX_SYS_CTRL_I2CR_MSK);
                 ...
                 const unsigned char *bytes)
{
    ...
    while(index < length)
    {
        ...
        i2c_error =
            NXP_I2C_Write(handlesLocal[handle].slave_address, size, bytes + index);
                        ...
                        unsigned char subaddress, unsigned short value)
{
    ...
    i2c_error =
        NXP_I2C_Write(handlesLocal[handle].slave_address, bytes2write, write_data);
    return classify_i2c_error(i2c_error);
}
enum Tfa98xx_Error
                                          ...
                                          const unsigned char *pSpeakerBytes)
{
    ...
    if(pSpeakerBytes != 0)
    {
        ...
        error = Tfa98xx_DspSetParamMultipleVarWait(
            handle_cnt, handles, MODULE_SPEAKERBOOST, SB_PARAM_SET_LSMODEL, length, pSpeakerBytes, TFA98XX_WAITRESULT_NTRIES_LONG);
    }
    else
    {
        error = Tfa98xx_Error_Bad_Parameter;
                          ...
                          int length, unsigned char *pSpeakerBytes)
{
    ...
    {
        error =
            Tfa98xx_DspGetParam(handle, MODULE_SPEAKERBOOST, paramId, length, pSpeakerBytes);
    }
    else
    {
        error = Tfa98xx_Error_Bad_Parameter;
                                           ...
                                           int *pbSupporttCoef)
{
    ...
    else if(error == Tfa98xx_Error_RpcParamId)
    {
        ...
        error =
            Tfa98xx_ReadRegister16(handle, TFA98XX_MTP + 6, &featureBits1);
        if(error == Tfa98xx_Error_Ok)
        {
            /* bit=0 means tCoef expected */
                       ...
                       const unsigned char *pConfigBytes)
{
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    error =
        Tfa98xx_DspSetParam(handle, MODULE_SPEAKERBOOST, SB_PARAM_SET_CONFIG, length, pConfigBytes);
    return error;
}
enum Tfa98xx_Error
                               ...
                               int length, const unsigned char *pConfigBytes)
{
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    error =
        Tfa98xx_DspSetParamMultiple(handle_cnt, handles, MODULE_SPEAKERBOOST, SB_PARAM_SET_CONFIG, length, pConfigBytes);
    return error;
}
enum Tfa98xx_Error
                       ...
                       const unsigned char *pPresetBytes)
{
    ...
    if(pPresetBytes != 0)
    {
		 ...
		 * interpreting/calculation */
        error =
            Tfa98xx_DspSetParam(handle, MODULE_SPEAKERBOOST, SB_PARAM_SET_PRESET, length, pPresetBytes);
    }
    else
    {
        error = Tfa98xx_Error_Bad_Parameter;
                                         ...
                                         unsigned char *pConfigBytes)
{
    ...
    if(pConfigBytes != 0)
    {
        ...
        error = Tfa98xx_DspGetParam(handle, MODULE_SPEAKERBOOST, SB_PARAM_GET_CONFIG_PRESET, length, pConfigBytes);
    }
    else
    {
        error = Tfa98xx_Error_Bad_Parameter;
                                         ...
                                         unsigned char *pPresetBytes)
{
    ...
    if(pPresetBytes != 0)
    {
        error = Tfa98xx_DspGetParam(handle, MODULE_SPEAKERBOOST, SB_PARAM_GET_CONFIG_PRESET, (TFA98XX_CONFIG_LENGTH + TFA98XX_PRESET_LENGTH), temp);
        if(error == Tfa98xx_Error_Ok)
        {
            int i;
            for(i = 0; i < length; i++)
                               ...
                               int length, const unsigned char *pPresetBytes)
{
    ...
    if(pPresetBytes != 0)
    {
		 ...
		 * interpreting/calculation */
        error =
            Tfa98xx_DspSetParamMultiple(handle_cnt, handles, MODULE_SPEAKERBOOST, SB_PARAM_SET_PRESET, length, pPresetBytes);
    }
    else
        error = Tfa98xx_Error_Bad_Parameter;
    return error;
}
enum Tfa98xx_Error Tfa98xx_SetVolume(Tfa98xx_handle_t handle, float voldB)
{
    enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
    unsigned short value;
    if(!handle_is_open(handle))
...
enum Tfa98xx_Error Tfa98xx_GetVolume(Tfa98xx_handle_t handle, float *pVoldB)
{
    enum Tfa98xx_Error error;
    unsigned short value;
    if(!handle_is_open(handle))
...
Tfa98xx_SetMute(Tfa98xx_handle_t handle, enum Tfa98xx_Mute mute)
{
    ...
    if(error != Tfa98xx_Error_Ok)
        return error;
    error =
        Tfa98xx_WriteRegister16(handle, TFA98XX_AUDIO_CTR, audioctrl_value);
    if(error != Tfa98xx_Error_Ok)
        return error;
    error =
                           ...
                           float b1, float b2, float a1, float a2)
{
    ...
    {
        error =
            Tfa98xx_DspSetParam(handle, MODULE_BIQUADFILTERBANK, (unsigned char)biquad_index, (BIQUAD_COEFF_SIZE * 3), bytes);
    }
    return error;
}
enum Tfa98xx_Error
                                   ...
                                   float a2)
{
    ...
    {
        error =
            Tfa98xx_DspSetParamMultiple(handle_cnt, handles, MODULE_BIQUADFILTERBANK, (unsigned char)biquad_index, (BIQUAD_COEFF_SIZE * 3), bytes);
    }
    return error;
}
enum Tfa98xx_Error
                              ...
                              float b[TFA98XX_BIQUAD_NUM * 5])
{
    ...
    {
        error =
            calcBiquadCoeff(b[i * 5], b[i * 5 + 1], b[i * 5 + 2], b[i * 5 + 3], b[i * 5 + 4], bytes + i * BIQUAD_COEFF_SIZE * 3);
    }
    if(error == Tfa98xx_Error_Ok)
    {
        error =
            Tfa98xx_DspSetParam(handle, MODULE_BIQUADFILTERBANK, 0 /* program all at once */, (unsigned char)(BIQUAD_COEFF_SIZE * TFA98XX_BIQUAD_NUM * 3), bytes);
    }
    return error;
}
enum Tfa98xx_Error
                                      ...
                                      float b[TFA98XX_BIQUAD_NUM * 5])
{
    ...
    {
        error =
            Tfa98xx_DspSetParamMultiple(handle_cnt, handles, MODULE_BIQUADFILTERBANK, 0 /* program all at once */, (unsigned char)(BIQUAD_COEFF_SIZE * TFA98XX_BIQUAD_NUM * 3), bytes);
    }
    return error;
}
enum Tfa98xx_Error
                                ...
                                const unsigned char *pBiquadBytes)
{
    ...
    if(error == Tfa98xx_Error_Ok)
    {
        error = Tfa98xx_DspSetParam(handle, MODULE_BIQUADFILTERBANK, (unsigned char)biquad_index, (BIQUAD_COEFF_SIZE * 3), pBiquadBytes);
    }
    return error;
}
enum Tfa98xx_Error
                                        ...
                                        const unsigned char *pBiquadBytes)
{
    ...
    if(pBiquadBytes == 0)
        return Tfa98xx_Error_Bad_Parameter;
    error =
        Tfa98xx_DspSetParamMultiple(handle_cnt, handles, MODULE_BIQUADFILTERBANK, (unsigned char)biquad_index, (BIQUAD_COEFF_SIZE * 3), pBiquadBytes);
    return error;
}
enum Tfa98xx_Error
Tfa98xx_DspBiquad_Disable(Tfa98xx_handle_t handle, int biquad_index)
{
    ...
    Tfa98xx_ConvertData2Bytes(BIQUAD_COEFF_SIZE, coeff_buffer, bytes);
    error =
        Tfa98xx_DspSetParam(handle, MODULE_BIQUADFILTERBANK, (unsigned char)biquad_index, (unsigned char)BIQUAD_COEFF_SIZE * 3, bytes);
    return error;
}
enum Tfa98xx_Error
                                  ...
                                  Tfa98xx_handle_t handles[], int biquad_index)
{
    ...
    Tfa98xx_ConvertData2Bytes(BIQUAD_COEFF_SIZE, coeff_buffer, bytes);
    error =
        Tfa98xx_DspSetParamMultiple(handle_cnt, handles, MODULE_BIQUADFILTERBANK, (unsigned char)biquad_index, (unsigned char)BIQUAD_COEFF_SIZE * 3, bytes);
    return error;
}
                 ...
                 const unsigned char *patchBytes)
{
    ...
    if(Tfa98xx_Error_Ok != error)
        return error;
    error =
        processPatchFile(handle, patchLength - PATCH_HEADER_LENGTH, patchBytes + PATCH_HEADER_LENGTH);
    return error;
}
...
checkRpcStatus(Tfa98xx_handle_t handle, int *pRpcStatus)
{
...
#ifdef OPTIMIZED_RPC
    {
        ...
        error =
            Tfa98xx_WriteData(handle, TFA98XX_CF_CONTROLS, sizeof(buffer), buffer);
    }
#else  /* OPTIMIZED_RPC */
    ...
    {
        error =
            Tfa98xx_WriteRegister16(handle, TFA98XX_CF_CONTROLS, cf_ctrl);
    }
    if(error == Tfa98xx_Error_Ok)
    {
    ...
    if(error == Tfa98xx_Error_Ok)
    {
        ...
        error =
            Tfa98xx_ReadData(handle, TFA98XX_CF_MEM, 3 /*sizeof(mem) */, mem);
    }
    if(error == Tfa98xx_Error_Ok)
               ...
               int num_bytes, const unsigned char data[])
{
    ...
    if(error == Tfa98xx_Error_Ok)
    {
        ...
        while((error == Tfa98xx_Error_Ok) && (remaining_bytes > 0))
        {
            ...
            error =
                Tfa98xx_WriteData(handle, TFA98XX_CF_MEM, chunk_size, data + offset);
...
waitResult(Tfa98xx_handle_t handle, int waitRetryCount)
{
    ...
    {
        error =
            Tfa98xx_ReadRegister16(handle, TFA98XX_CF_STATUS, &cf_status);
        tries++;
    }
    /* don't wait forever, DSP is pretty quick to respond (< 1ms) */
    while((error == Tfa98xx_Error_Ok) && ((cf_status & 0x0100) == 0) && (tries < waitRetryCount));
                                   ...
                                   int waitRetryCount)
{
    ...
    for(i = 0; (i < handle_cnt) && (error == Tfa98xx_Error_Ok); ++i)
        error =
            writeParameter(handles[i], module_id, param_id, num_bytes, data);
    /* 2) wake up the DSP and let it process the data */
    for(i = 0; (i < handle_cnt) && (error == Tfa98xx_Error_Ok); ++i)
                    ...
                    int num_bytes, unsigned char data[])
{
    ...
    if(error == Tfa98xx_Error_Ok)
    {
        ...
        /* don't wait forever, DSP is pretty quick to respond (< 1ms)*/
        while((error == Tfa98xx_Error_Ok) && ((cf_status & 0x0100) == 0) && (tries < TFA98XX_WAITRESULT_NTRIES));
    ...
    if(error == Tfa98xx_Error_Ok)
    {
        ...
        while((error == Tfa98xx_Error_Ok) && (remaining_bytes > 0))
        {
            ...
            error = Tfa98xx_ReadData(
                handle, TFA98XX_CF_MEM, chunk_size, data + offset);
            remaining_bytes -= chunk_size;
            offset += chunk_size;
        }
    }
                        ...
                        struct Tfa98xx_StateInfo *pInfo)
{
    ...
    {
        error =
            Tfa98xx_DspGetParam(handle, MODULE_FRAMEWORK, FW_PARAM_GET_STATE, 3 * stateSize, bytes);
    }
    else
    {
        ...
        stateSize = 8;
        error =
            Tfa98xx_DspGetParam(handle, MODULE_SPEAKERBOOST, SB_PARAM_GET_STATE, 3 * stateSize, bytes);
    }
    Tfa98xx_ConvertBytes2Data(3 * stateSize, bytes, data);
    /* /2^23*2^(SPKRBST_AGCGAIN_EXP) */
    pInfo->agcGain = (float)data[0] / (1 << (23 - SPKRBST_AGCGAIN_EXP));
    /* /2^23*2^(SPKRBST_LIMGAIN_EXP) */
    pInfo->limGain = (float)data[1] / (1 << (23 - SPKRBST_LIMGAIN_EXP));
    /* /2^23*2^(SPKRBST_HEADROOM)    */
    pInfo->sMax = (float)data[2] / (1 << (23 - SPKRBST_HEADROOM));
    /* /2^23*2^(def.SPKRBST_TEMPERATURE_EXP) */
    pInfo->T = data[3] / (1 << (23 - SPKRBST_TEMPERATURE_EXP));
    pInfo->statusFlag = data[4];
    /* /2^23*2^(SPKRBST_HEADROOM)        */
    pInfo->X1 = (float)data[5] / (1 << (23 - SPKRBST_HEADROOM));
    /* /2^23*2^(SPKRBST_HEADROOM)        */
    pInfo->X2 = (float)data[6] / (1 << (23 - SPKRBST_HEADROOM));
    /* /2^23*2^(SPKRBST_TEMPERATURE_EXP) */
    pInfo->Re = (float)data[7] / (1 << (23 - SPKRBST_TEMPERATURE_EXP));
    return error;
}
enum Tfa98xx_Error
Tfa98xx_DspGetCalibrationImpedance(Tfa98xx_handle_t handle, float *pRe25)
{
    ...
    *pRe25 = 0.0f; /* default 0.0 */
    ...
    if(error == Tfa98xx_Error_Ok)
    {
        ...
        error = Tfa98xx_DspGetParam(handle, MODULE_SPEAKERBOOST, SB_PARAM_GET_RE0, 3, bytes);
    }
    if(error == Tfa98xx_Error_Ok)
    {
        Tfa98xx_ConvertBytes2Data(3, bytes, data);
/* /2^23*2^(def.SPKRBST_TEMPERATURE_EXP) */
        *pRe25 = (float)data[0] / (1 << (23 -
                      ...
                      unsigned char outdata[])
{
    ...
    /* 5) read the resulting data */
    error =
        Tfa98xx_WriteRegister16(handle, TFA98XX_CF_MAD, 2 /*start_offset */);
    if(error != Tfa98xx_Error_Ok)
        return error;
                      ...
                      unsigned short start_offset, int num_words, int *pValues)
{
    ...
    error =
        Tfa98xx_DspExecuteRpc(handle, 0 /* moduleId */, 5 /* paramId */, sizeof(input_bytes), input_bytes, num_words * 3, output_bytes);
