...
enum Tfa98xx_Error Tfa9890_DspSystemStable(Tfa98xx_handle_t handle, int *ready)
{
    ...
    *ready =
        (status &
         (TFA98XX_STATUSREG_AREFS_MSK | TFA98XX_STATUSREG_CLKS_MSK)) == (TFA98XX_STATUSREG_AREFS_MSK | TFA98XX_STATUSREG_CLKS_MSK);
...
errorExit:
    *ready = 0;
    assert(error); /* an error here can be considered
				   to be fatal */
    return error;
}
/*
...
enum Tfa98xx_Error Tfa9890_DspReset(Tfa98xx_handle_t handle, int state)
{
    ...
    if(Tfa98xx_Error_Ok == error)
    {
        ...
        error =
            Tfa98xx_WriteRegister16(handle, TFA98XX_CURRENTSENSE4, value);
        if(error)
            return error;
    }
    ...
    value = state ? (value | TFA98XX_CF_CONTROLS_RST_MSK) : (value & ~TFA98XX_CF_CONTROLS_RST_MSK);
